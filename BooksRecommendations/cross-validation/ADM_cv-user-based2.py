...
# (only ratings estimated with user-based)
out_file = open("ADM_cv-user-based2.txt","w")
...
for i in range(5):
    ...
    #for row in range(10):
        ...
        if rating!=0:
            ...
            print('vicini:\n',nearest_users)
            # let's find the neighbours' ratings for the book
            ratings = []
    ...
    print(RMSE[-1])
    print('book estimated with user-based:',f,'   over:',n)
    string = 'books estimated with user-based: '+str(f)+'   over: '+str(n)+'\n\n'+'RMSE: '+str(RMSE[-1])+'\n\n'
    out_file.write(string)
