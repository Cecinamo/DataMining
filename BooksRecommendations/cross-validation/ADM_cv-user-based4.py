...
# (ratings estimated with the mean of the user's ratings)
out_file = open("ADM_cv-user-based4.txt","w")
...
for i in range(5):
    ...
    #for row in range(10):
        ...
        if rating!=0:
            ...
            if len(ratings)>0:
                ...
                r = [int(v) for v in ratings_users[i][user].values() if int(v)!=0]
                if len(r)>0:
                    f+=1
                    r_hat = np.mean(r)
                else:
    ...
    print(RMSE[-1])
    print('books estimated with the mean of the user\'s ratings:',f,'   su:',n)
    string = 'books estimated with user mean: '+str(f)+' over:'+str(n)+'\n\n'+'RMSE: '+str(RMSE[-1])+'\n\n'
    out_file.write(string)
