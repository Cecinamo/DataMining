...
# let's save the result in a text file

out_file = open("ADM_cv-user-based1.txt","w")
...
for i in range(5):
    ...
    # for each row in the test set we have a user and a book and we have to estimate the rating
    # for that user, for that book. we use the mean of the neighbour's ratings for that book.

    # number of rows estimated with user-based
    f = 0
    ...
    for row in range(test[i].shape[0]):
        ...
        # we are not going to estimate values equal to 0     
        if rating!=0:
            notnull = True
            #print()
            n+=1
            ...
            #print('nearest:',nearest[i][user])
            # let's find the user's neighbours
            nearest_users = []
            if user in nearest[i].keys():
                nearest_users = set(nearest[i][user].keys()).difference(user)
            #print('neighbours:\n',nearest_users)
            # let's find the neighbours' ratings for the book troviamo i voti dei vicini per book
            ratings = []
            for nu in nearest_users:
                # the neighbour has rated the book
                if book in ratings_users[i][nu].keys():
                    #print('neighbour:', nu, 'has rated the book:',ratings_users[i][nu][book])
    ...
    print('RMSE: ',RMSE[-1])
    print('books estimated with mbooks:',f,'   over:',n)
